- [Data Folder](#data-folder)
  ...
  - [File Explainers](#file-explainers)
    ...
    - [Search Autocomplete Index - `building-search-index.csv`](#search-autocomplete-index---building-search-indexcsv)
...
Source: https://github.com/ChicagoCityscape/gis-data/blob/master/d_chicago_energy_benchmark_buildings_permanent_20240115.geojson
Contains corrected geocoding information for buildings that report emissions data. The file is used
during data processing to overrwrite incorrect locations from the city's raw data. To update, check the
timestamp and simply download the file directly from github.
...
### Search Autocomplete Index - `building-search-index.csv`

A lightweight index powering the search autocomplete, with one row per building
and just the columns needed for matching and display: `id`, `name`, `address`,
and `type`. It's generated by `scripts/generate_search_index.py` from
`building-benchmarks.csv`.

Each row stores the building `id` rather than a full URL. The frontend links to
`/building-id/:id`, which auto-redirects to the building's canonical
`/building/:slug` page - so we don't have to replicate Gridsome's slugify logic
in Python. `gridsome.server.js` copies this file into `static/` at build time so
the autocomplete can fetch it from the site root on demand, rather than bundling
~3.5k records into the initial page JS.

We use CSV instead of JSON so the column names aren't repeated on every row,
which keeps the file small and gives clean per-row git diffs when the data
changes.
