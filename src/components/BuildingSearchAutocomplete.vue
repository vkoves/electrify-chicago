 ...
 * from /building-search-index.csv (generated by the data pipeline, see
 * src/data/scripts/generate_search_index.py), so it's ready by the time the
 * user types but never bloats the initial page load.
 *
...
export default class BuildingSearchAutocomplete extends Vue {
  ...
  async loadIndex(): Promise<void> {
    ...
    try {
      const response = await fetch('/building-search-index.csv');
      const csvText = await response.text();

      this.normalizedIndex = this.parseIndexCsv(csvText).map((item) => ({
        item,
  ...
  /**
   * Parse the search index CSV (columns: id, name, address, type) into search
   * items, deriving each building's path from its id. The first line is the
   * header and is skipped.
   */
  private parseIndexCsv(csvText: string): Array<IBuildingSearchIndexItem> {
    const items: Array<IBuildingSearchIndexItem> = [];
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const [id, name, address, type] = parseCSVLine(line);
