...
export default class ScatterplotGraph extends Vue {
  ...
  mounted() {
    ...
    const emissionsData: HunterPoint[] = [
    ...
    const svg = d3
      ...
      const topAndBottomEmissionsValues = d3.extent(emissionsData, (d: HunterPoint) => d.value) as [number, number]
      const yAxisLinearlyScaledLineWeMade = d3.scaleLinear().domain(topAndBottomEmissionsValues).range([height, 0])
      
      // or you can see here, it's less explicit but shorter code
      const topAndBottomYears = d3.extent(emissionsData, (d: HunterPoint) => d.year) as [number, number]
      const xAxisLinearlyScaledLineWeMade = d3.scaleLinear(topAndBottomYears, [0, width])
      ...
      .call(
        ...
        d3.axisBottom(xAxisLinearlyScaledLineWeMade) // draws the x line axis at the bottom
      ...
      svg
        ...
        .attr(
          ...
          d3
            .line<HunterPoint>()
            .x((d: HunterPoint) => xAxisLinearlyScaledLineWeMade(d.year))
            .y((d: HunterPoint) => yAxisLinearlyScaledLineWeMade(d.value))
        )
      ...
      svg
        ...
        .attr('cx', (d: HunterPoint) => xAxisLinearlyScaledLineWeMade(d.year)) // the center point of the x axis of the circle
        .attr('cy', (d: HunterPoint) => yAxisLinearlyScaledLineWeMade(d.value)) // the center point of the y axis of the circle
        .attr('r', 7)
        .attr('fill', '#69b3a2')
    }
  }
...
<style lang="scss" scoped> #my_dataviz { margin-top: 2rem; } </style>
