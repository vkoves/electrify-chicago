...
def test_generate_percentile_grade():
    ...
    assert all(
        percentiles_rev[i] >= percentiles_rev[i + 1]
        for i in range(len(percentiles_rev) - 1)
    )
...
def test_generate_consistent_reporting_grade():
    ...
    test_data = pd.DataFrame(
        {
            ...
            "ReportingStatus": [
                "Submitted",
                "Submitted",
                "Submitted",  # ID 1: 100% reporting
                "Submitted",
                "Not Submitted",
                "Submitted",  # ID 2: 67% reporting
                "Not Submitted",
                "Not Submitted",
    ...
    assert id2_row["SubmittedRecordsPercentileGrade"].values[0] == pytest.approx(
        66.67, 0.1
    )
    ...
    assert id3_row["SubmittedRecordsPercentileGrade"].values[0] == pytest.approx(
        33.33, 0.1
    )
    ...
    with patch("pandas.read_csv", return_value=historical_data):
        ...
        for building_id in [1, 2]:
            for year in [2021, 2022]:
                ...
                building_year_data = result[
                    (result["ID"] == building_id) & (result["DataYear"] == year)
                ]
...
def test_building_grade_examples():
    ...
    historical_data = pd.DataFrame(
        {
            ...
            "GHGIntensity": [
                ...
                5,
                5,
                5,  # Building 1: Low GHG
                15,
                15,
                15,
                15,
                15,  # Building 2: Medium GHG
                30,
                30,
                30,
                30,
            ...
            "ElectricityUse": [
                ...
                1000,
                1000,
                1000,  # Building 1: All electric
                500,
                500,
                500,
                500,
                500,  # Building 2: 50/50 mix
                0,
                0,
                0,
                0,
                0,
            ],  # Building 3: No electric
            "NaturalGasUse": [
                0,
                0,
                0,
    ...
    with patch("pandas.read_csv", return_value=historical_data):
        with patch(
            "src.data.scripts.grade_buildings.grade_ghg_intensity_energy_mix_all_years"
        ) as mock_grade:
            ...
            with patch(
                "src.data.scripts.grade_buildings.generate_consistent_reporting_grade"
            ) as mock_reporting:
