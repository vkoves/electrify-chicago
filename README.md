...
# Generate only property type social images
docker compose run --rm electrify-chicago yarn gen-social-imgs-property-types
...
**Property Type Social Cards** (at `/property-type-social-card/{property-type-slug}`) featuring:

- Property type name (pluralized, e.g., "Office Buildings", "Data Centers")
- Building count for that type
- Hero section with the largest buildings of that type
- Electrify Chicago branding
