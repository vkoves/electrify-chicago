...
```bash
# Full build with social images (default)
docker-compose run --rm electrify-chicago yarn build

# Build site only (without generating social images)
docker-compose run --rm electrify-chicago yarn build-site-only

# Generate social images only (after site is built)
docker-compose run --rm electrify-chicago yarn generate-social-images

# Generate test social images only (for development/testing)
docker-compose run --rm electrify-chicago yarn generate-test-social-images
```
...
The images are generated by screenshotting custom Vue components at `/social-card/{building-id}` using Puppeteer, ensuring consistent design with the rest of the site.

**Development Tip**: Use `yarn generate-test-social-images` to quickly generate social images for just 3 test buildings (Crown Hall, Marina Towers, and 445 W Erie) when developing or testing social card changes. This is much faster than generating images for all buildings.
